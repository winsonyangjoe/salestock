...
class MY_Controller extends CI_Controller {
    ...
    public function __construct() {
        ...
        if($this->router->fetch_class()!="order")
        {
            if (!$this->is_logged_in() && !$this->is_login_page()) {
                redirect('user/login?' . http_build_query(['redirect' => $_SERVER['REQUEST_URI']]));
            }
        }
    }
    ...
    protected function web_view($view, $data = [], $header_data = [], $footer_data = []) {
        $this->load->view($view, $data);
    }
}